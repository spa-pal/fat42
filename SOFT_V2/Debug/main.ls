...
4312  07d1               L3022:
4313                     ; 871 else	if(link==ON)				//если есть связьvol_i_temp_avar
4315  07d1 b66d          	ld	a,_link
4316  07d3 a155          	cp	a,#85
4317  07d5 265c          	jrne	L1712
4318                     ; 873 	if((flags&0b00100000)==0)	//если нет блокировки извне
4320  07d7 b605          	ld	a,_flags
4321  07d9 a520          	bcp	a,#32
4322  07db 264a          	jrne	L1322
4323                     ; 875 		if(((flags&0b00011010)==0b00000000)) 	//если нет аварий или если они заблокированы
4325  07dd b605          	ld	a,_flags
4326  07df a51a          	bcp	a,#26
4327  07e1 260b          	jrne	L3322
4328                     ; 877 			pwm_u=vol_i_temp;					//управление от укушки + выравнивание токов
...
4338  07ee               L3322:
4339                     ; 880 		else if(flags&0b00011010)					//если есть аварии
4341  07ee b605          	ld	a,_flags
4342  07f0 a51a          	bcp	a,#26
4343  07f2 2706          	jreq	L5322
4344                     ; 882 			pwm_u=0;								//то полный стоп
4346  07f4 5f            	clrw	x
...
4387  0827               L1322:
4388                     ; 902 	else if(flags&0b00100000)	//если заблокирован извне то полное выключение
...
4942                     ; 1053 	if(((jp_mode==jp0)&&!(flags&0b01000000))||(jp_mode==jp3))	flags|=0b00000100; //поднять бит подогрева 
...
4960  0aa0               L3332:
4961                     ; 1055 else if (tsign_cnt<=5) flags&=0b11111011;	//Сбросить бит подогрева
4963  0aa0 9c            	rvf
...
5130                     ; 1082 	if(umax_cnt>=10)flags|=0b00001000; 	//Поднять аварию по превышению напряжения
5132  0b41 9c            	rvf
...
7689  1541 85            	popw	x
7690  1542               L7533:
7691                     ; 1907 	if(mess[8]==MEM_KF4)	//MEM_KF4 передают УКУшки там, где нужно полное управление БПСами с УКУ, включить-выключить, короче не для ИБЭП
...
9190                     ; 2384 		pwr_drv();		//воздействие на силу
...
9229                     ; 2407 	  pwr_hndl();		//вычисление воздействий на силу
9231  1b2e cd06f1        	call	_pwr_hndl
9233                     ; 2408 		temper_drv();			//вычисление аварий температуры
