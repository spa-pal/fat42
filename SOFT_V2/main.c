//Ветка для быстрой вольтдобавки
...
if(jp_mode!=jp3)
	...
	if(link_cnt==90)flags&=0xc1;		//если оборвалась связь первым делом сбрасываем все аварии и внешнюю блокировку
	if(link_cnt==100)
		{
		link=OFF;
...
if(alfa_pwm_max_i_cnt)
	{
	alfa_pwm_max_i_cnt--;
	if(alfa_pwm_max_i_cnt==0)alfa_pwm_max_i_cnt__=0;
	alfa_pwm_max_i=1460;	
	}
else 
	{
